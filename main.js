...
const INDEX_PRESETS = [
  ...
  { symbol: 'sz399618', name: '深证医药', group: 'sector' },
  { symbol: 'sh000037', name: '上证医药', group: 'sector' },
  { symbol: 'sz399986', name: '中证银行', group: 'sector' },
  { symbol: 'sz399975', name: '证券公司', group: 'sector' },
  { symbol: 'sz399809', name: '保险主题', group: 'sector' },
  ...
  { symbol: 'sz399967', name: '中证军工', group: 'sector' },
  { symbol: 'sz399808', name: '中证新能', group: 'sector' },
  { symbol: 'sz399976', name: 'CS新能车', group: 'sector' },
  { symbol: 'sz399417', name: '新能源车', group: 'sector' },
  { symbol: 'sh000827', name: '中证环保', group: 'sector' },
  { symbol: 'sz399998', name: '中证煤炭', group: 'sector' },
  { symbol: 'sh000928', name: '中证能源', group: 'sector' },
...
// 旧版单列表配置 → 多列表。
// 旧 [stocks] → 列表 l1「自选股」；旧 [indexes] → 列表 l2「板块指数」（非空时才建）。
...
function migrateLegacy(sections) {
  ...
  // v1.4.x 的定时汇总是「全局一封」（把自选股 + 指数合成一封）。
...
// ---------- 邮件定时汇总（digest）----------
// 用户要求：每隔 N 分钟，把"当前全部行情"（当前列表的标的）汇总成一封邮件发出，开关可选。
...
function closeListWidget(listId) {
  const l = getList(listId);
  if (l) { l.visible = false; saveConfig(); }
  const w = widgetWindows.get(listId);
  if (w && !w.isDestroyed()) { try { w.hide(); } catch (e) { logErr('closeListWidget', e); } }
}

// 显示单个列表的小组件（托盘 → 监控列表 → 显示）：把「启动时显示」置 true 并显示窗口。
function showListWidget(listId) {
  const l = getList(listId);
  if (!l) return;
  l.visible = true;
  saveConfig();
...
// 隐藏单个列表的小组件（托盘 → 监控列表 → 隐藏）：同样持久化「启动时显示」，
// 这样设置页的勾选状态与托盘菜单始终一致，不会"明明关掉了下次又冒出来"。
function hideListWidget(listId) {
  const l = getList(listId);
  if (l) { l.visible = false; saveConfig(); }
  const w = widgetWindows.get(listId);
  if (w && !w.isDestroyed()) { try { w.hide(); } catch (e) { logErr('hideListWidget', e); } }
}
...
// 小组件右上角的「×」= 关闭【这一个】列表的小组件，不再退出整个程序。
// 早期版本这里直接退出整个程序，导致"关掉一个 widget 全部都没了"，多列表下完全不可用。
// 语义：该列表的「启动时显示」置 false（持久化）并隐藏它的窗口；
// 想让它回来：托盘 → 监控列表 → 该列表 → 显示；想退出程序：托盘 → 退出程序。
ipcMain.handle('widget-quit', (e) => {
  try {
    const win = BrowserWindow.fromWebContents(e.sender);
    const listId = win ? winListId.get(win) : null;
    if (listId && getList(listId)) { closeListWidget(listId); return { ok: true, listId }; }
  } catch (err) { logErr('widget-quit', err); }
  return { ok: false };
});
...
function createTray() {
  ...
  const listItems = (store.lists || []).map(l => ({
    ...
    submenu: [
      ...
      { label: '隐藏', click: () => { try { hideListWidget(l.id); } catch (e) { logErr('tray-hide-list', e); } } },
    ],
  }));
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { label: '退出程序（关闭全部列表）', click: () => app.quit() },
  ]);
...
const CHANGELOG = [
  ...
  },
  {
    ...
    items: [
      '修复：设置页「+ 新建列表」点了没反应、加不出新列表 —— 根因是这段代码调用了浏览器原生的输入弹窗（Electron 渲染进程不支持，一调就直接报错），且报错发生在异常保护之外，于是表现为按钮完全静默',
      '现在的做法：点「+ 新建列表」立即建好一个新列表并自动切过去，光标直接落到「列表名称」输入框，输入即改名（少一步弹窗，也不再依赖任何原生对话框）',
      '再次明确多列表的分工：每个列表 = 独立的悬浮小组件窗口，标的（股票 / 基金 / 指数）各自一份、异动阈值与收件邮箱各自一份、定时汇总的开关与推送方式也各自一份 —— 想给「自选股」和「板块指数」发不同预警、发给不同人，就用这个功能',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      '新增多监控列表：每个列表一个独立悬浮窗，可分别监控自选股、板块指数、基金等，互不干扰',
      '小组件右上角「×」只关闭当前列表的小组件（原先会直接退出整个程序）；退出程序请用托盘菜单',
      '指数清单扩充并分组：11 个宽基指数 + 32 个板块（行业/主题）指数，勾选即加入当前列表',
      '修正指数勾选区里混进个股的问题：现在只列指数，个股 / ETF / 可转债不再出现在这里',
  ...
  },
  {
    ...
    items: [
      ...
      '新增指数监控：上证指数 / 深证成指 / 创业板指 / 沪深300 / 中证500 / 科创50 / 恒生指数，按需勾选',
