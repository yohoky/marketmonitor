...
function renderIdxGrid() {
  ...
  // 预设 + 当前列表里出现过的额外指数代码（保证手写在 config.ini 里的指数不丢）
  const all = state.presetIndexes.slice();
  for (const s of symbols) {
    const key = String(s.symbol).toLowerCase();
    if (!all.some(p => String(p.symbol).toLowerCase() === key)) all.push({ symbol: s.symbol, name: s.name });
  }
  idxGrid.innerHTML = '';
  all.forEach((p) => {
...
document.getElementById('new-list-btn')?.addEventListener('click', async () => {
  const name = prompt('新列表名称（例如：基金 / 港股 / 大盘指数）', '列表 ' + (state.lists.length + 1));
  if (name === null) return;
  try {
