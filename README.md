...
**v1.4.0 · MIT License · 无需账号 · 免费无 Key**
...
| 功能 | 说明 |
|------|------|
| 📈 自选管理 | 支持 **A 股 / 基金(ETF) / 可转债**：输入 `600519`、`510300`(基金)、`128050`(可转债) 或 `sh600519` 自动识别沪深；支持批量粘贴（逗号/空格/换行） |
| 🔤 名称自动补全 | 只有代码没有名称时，自动从行情 / 接口解析成中文名并落库（列表优先展示名称）|
| 🔔 异动提醒 | 涨幅/跌幅**分别设置**阈值（默认各 3.9%），**默认仅在 A 股盘中提醒**（北京时间 09:25–11:35 / 12:55–15:05，周末除外，可在设置里关闭）；触发时系统通知 + 提示音 + 卡片闪烁，冷却去重不刷屏 |
...
- 已支持 **A 股 / 基金(ETF) / 可转债**；北交所（4 / 8 开头）仍需手动加 `sh/sz` 前缀
- 停牌股票显示最后一次有效数据
- 无网络时占位为 `--`，恢复后自动刷新
